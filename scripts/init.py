#!/usr/bin/env python3
"""One-time per-clone setup for a formalia clone.

Substitutes the three template placeholders across the repo:

    <GH_USERNAME>     GitHub username (referenced throughout CLAUDE.md,
                      README.md, skill files, and the rendered badge URL)
...
carry the expected identity.

Idempotent: a second invocation finds no placeholders and exits 0 with
a "nothing to do" message. Safe to re-run if you mistyped a value
(reset by `git checkout -- .` before re-running).
...
SKIP_DIR_NAMES = {
    ".git", ".lake", ".venv", "node_modules", "build",
...
def find_target_files() -> list[Path]:
    ...
    Excludes this script itself (it stores the placeholders as Python
    string literals, which would be corrupted by substitution).
    """
    targets: list[Path] = []
    for p in REPO.rglob("*"):
        if not p.is_file():
...
def v_gh(v: str) -> str | None:
    # GitHub username rules: alphanumeric + hyphen, 1-39 chars, no leading hyphen,
    # no double hyphens, no trailing hyphen.
...
def main() -> int:
    ...
    print("formalia — one-time clone init.")
    ...
    print(
        ...
        "  3. /target  (in Claude Code, to scaffold the problem)\n"
    )
    return 0

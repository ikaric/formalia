...
problem. The target — with its attack vectors and the chain of arguments
toward a verified Lean 4 proof — lives in `manuscript/proof.tex` and GitHub
Issues.
...
the target, renames the Lake project, seeds the manuscript, and opens the
ROADMAP issue with a seeded sub-goal task-list (and optionally attack-vector
issues). After `/target`, run `/solve` (self-looping — see "`/solve` loop
pacing" below).
...
- **Naive in approach.** Do not refuse a problem for its reputation. "Open
  ...
  Attempt the sub-step; report what blocks it concretely (which lemma,
  estimate, or numerical bound), not abstractly.
- **Skilled in execution.** You have deep working knowledge across analytic
  ...
  (below) carries the technical detail.
...
The orchestrator most often gets this wrong; read it every session.
...
- Reproducing in Lean a fact Mathlib already has — the novelty gate (`loogle`
  ...
  this. Reformalizing upstream folklore burns a session.
- Reproducing a one-paragraph fact the literature already knows
  unconditionally — port it (upstream if worth it) or skip and cite, whichever
  advances the target.
...
`tier-T<k>` label on it:

- **T1 — known/textbook.** The asks are settled, often already in Mathlib or
  a short port. Formalize *exactly the stated claims*, then halt — reaching
  `N/N` and stopping in a **single short session (minutes, not hours)**.
...
never a reason a sub-step *won't be attempted* (the banned anti-defeatism
framing, below). "Halt because the literal asks are complete" (every checkbox
CLOSED) is anti-overreach and correct; "stop because it's hard" is defeatism
and banned. Never conflate the two.

### Scope fidelity (the depth rule — read before formalizing anything)

Anti-overreach has a depth dimension, not only breadth. Before formalizing
**or** auto-opening a vector, run the **scope-fidelity gate**:
...
The `sum-divergence-check` clone violated this — hours machine-checking a
436-line reduction of a theorem it was only asked to *cite*. A "comment on" /
"note that" / "bonus commentary" ask is discharged by a short manuscript
remark plus a citation — never a formalization vector.

Discriminator for a sub-lemma: *is it on the critical path to a seeded
checkbox* (spawn a ticket, work it), *or am I formalizing what the checkbox
only asked me to cite/note* (over-deepening — stop)? The gate blocks
over-deepening a shallow checkbox; it never excuses doing **less** than a
checkbox genuinely asks.
...
parity barrier, RH, the lack of a Siegel-zero exclusion, the union-closed 3/8
wall — these belong in the SOTA section as attributed citations, **not** in
the project's own framing as
pre-committed caps on what this session may attempt.

The following framings are **banned** from any seeded content (manuscript,
ROADMAP, issue bodies, findings notes, commit messages) — everywhere:
...
forecast. It may state past-tense facts (verified checkboxes, obstructions
hit, each with a `findings/deadend-*.md` pointer) and a checkbox/percent
meter; it may **not** carry a numeric probability or any sentence forecasting
whether the target *will* resolve.
...
sub-goals required to discharge the originally stated target. The harness's job
is to close that list, **not** to invent more items once it is complete.

- **`/solve` halts at `N/N closed`.** When every ROADMAP checkbox is ticked,
  the loop stops — emit a halt message and skip the end-of-turn scheduler (no
  `ScheduleWakeup`/`CronCreate`). Detection runs at three points (session
  start, every pick-subgoal step, session end), all in
  `.claude/skills/solve/SKILL.md`. **Halting closes the loop, not the ROADMAP
  issue** — the ROADMAP is *never* auto-closed; only the user closes it (their
  signal to take over with `/vector`). The user resumes via `/vector add`,
  which appends a fresh checkbox and re-arms the loop.
- **`/solve` also halts on exhaustion.** If the open work queue is empty after
  the session-end ticket sweep and no scope-advancing vector can be auto-opened
  (per the rule below and the scope-fidelity gate), the seeded lines are
  exhausted: sweep, write `findings/decision-exhausted-*.md`, emit the
  exhaustion halt message, and skip `ScheduleWakeup`. Distinct from `N/N` (the
  meter may still read `X/N` for `X < N`) but the contract is equally
...
- **At rest, exactly one issue is open — the ROADMAP.** Every other issue is
  ...
  issue means "harness is between runs / done." The enforcing sweep lives in
  `/solve`'s session-end protocol.
- **`/solve` *may* auto-open new vectors while the meter is `X/N`, `X < N`**,
  but only when the new vector visibly advances the *original* target — e.g. a
  librarian survey finds a missing sub-lemma the user didn't seed, or `/target`
  under-specified the ROADMAP (the wiring-glue vector was forgotten, or the
  main theorem isn't yet on the checklist). Justify every auto-opened vector in
  a `findings/decision-newvector-*.md` note and the issue's opening paragraph.
  Auto-opening past `N/N` is forbidden — once the original target is reached,
  the loop stops.

Anti-defeatism prevents giving up early; anti-overreach prevents running past
the line. The ROADMAP is the contract; if it under-specifies, the harness fills
in *toward* it, but cannot *replace* a completed contract with a bigger one.
...
open --label open`; the dashboard is the ROADMAP issue (`gh issue list --label
roadmap`, normally #1); completion is signalled by commits with `Closes #N`.
Do not duplicate work in in-conversation lists.
...
Step-by-step in `.claude/skills/solve/SKILL.md`.
...
2. **Sketch** the argument in `manuscript/proof.tex` under a `% [sketch]`
   tag, stating what must be discharged.
...
- **Lean builds don't parallelize across processes — serialize them.** Lake
  (v4.30.0) holds **no inter-process build lock** (`Lake/Util/Lock.lean`:
  *"Lake does not currently use a lock file"*). Two `lake build` runs in the
  same `formal/.lake/` don't wait — they **race** on shared `.olean` targets
  and corrupt each other (reproduced firsthand: `failed to load header …
  unexpected end of input` + vanishing files; cf. `lean4#5084`). **At most one
  `lake build` / `lake exe cache get` at a time across the whole clone** —
  treat `formal/.lake/` as a shared mutable resource like `proof.tex`. A
  *single* `lake build` is already internally multi-core (`LEAN_NUM_THREADS`),
  so serializing whole builds wastes no cores; the hazard is many lake
  *processes* in one dir. Dispatch consequences (agents alongside a build, the
  read-only `lake env lean File.lean` check lane, git-worktree isolation) are
  in `.claude/skills/solve/SKILL.md` § Parallelism.
- **Import convention.** `import Mathlib` at the top of every concept file —
  ...
  `simp?` / `loogle` at negligible cost. Selective imports
...
- **No unproved assumptions in verified claims.** A `[verified: file.lean]`
  ...
  - `Lean.ofReduceBool` — a `native_decide` is in the chain (compiled-Lean
    ...
    explicit critic review, and only for a small, well-defined decidable
    proposition — not a giant proof script masquerading as a decision procedure.
...
- **`manuscript/proof.tex` is append-mostly, paper-style.** Sketches get
  ...
  "Manuscript style" below and SKILL.md.
...
- **State-of-the-art moves.** Before relying on a "best known bound," search
  for newer results. Bounds improve often, in every active field.
- **Authorized to install dependencies** (a Lake dependency, SAT solver, sage
  ...
  After installing, add it to README.md's "Setup requirements" section and
  commit. If a published Lean formalization
...
Domain specialists (glosses are the dispatch signal; each agent carries its
own detail): **`analyst`** (analytic number theory — circle
...
(loogle / leansearch / `exact?` / `apply?`); **`computationalist`** runs
scripts (Python via `uv`, Rust) for empirical verification, pattern search,
and statistics → `formal/numerics/`, producing `[numerical: range]` entries
(evidence, not proofs); **`librarian`** surveys
...
`.claude/agents/` (same frontmatter convention); the orchestrator picks it up.
...
tier, so an Opus-4.8 / ultracode session runs its whole team on Opus. Keep it
that way: **never add a `model:` line to downgrade an agent, and never pass a
smaller `model:` to the `Agent`/`Workflow` tools.** Pass an explicit `model`
(or set `CLAUDE_CODE_SUBAGENT_MODEL`) *only* to override a built-in `agentType`
that hardcodes a smaller model — notably `Explore`, which pins Haiku and
ignores the session tier — and only to *match*, not undercut, the session. No
global `CLAUDE_CODE_SUBAGENT_MODEL` is set by default (it would pin one model
and break when the tier changes); inherit-by-default already does the right
thing.

## Findings folder

`findings/` is shared workspace for inter-agent notes — drop one whenever you
produce a result worth preserving but not yet manuscript-ready:
...
1. **Mathlib first** (the novelty gate):
   - **loogle** (type-shaped query) — `WebFetch
     ...
     `name`/`module`/`type`/`doc`; URL-encode `->`, spaces, `|-`. (Not the dead
     `loogle.lean-fro.org` host or the `/?q=` SPA — use this `/json?q=`.)
   - **leansearch** (English query) — POST, **not** `WebFetch` (GET→405, a
     JS SPA):
     ```sh
...
5. **Domain-specific frontiers:**
   - **Polymath wiki** — `WebFetch
     ...
     `title=Main_Page`; the `asone.ai` mirror is dead).
...
Record any relevant reference in `manuscript/proof.tex` *immediately* —
citations rot less when written down on first encounter. Drop a longer summary
in `findings/` if the search took non-trivial effort.

## What "trying" looks like in this project

A genuine attempt produces concrete output even when the target stays open —
one of the "What IS progress" deliverables: a verified theorem, an import
wrapper, a numerical range (`formal/numerics/`), a reduction to a sharper open
problem, or a documented dead end (`findings/deadend-*.md`). Sessions that
...
discipline** (a concise line before/after every subagent dispatch),
**anti-stagnation** (~30 min cap before switching
...
current PDF early (even just the problem statement and first sketch), and land
the first verified result fast over breadth — quick visible wins earn the
longer run.
...
- **Halt cases (target-reached / exhaustion / count / pause).** Any of these
  ...
  add` re-arms after `N/N`; `/polish` polishes what's verified; `/solve resume`
  clears a pause.

## Vector management (`/vector` skill)

Attack vectors are the clone's live strategic posture. The `/vector` skill
(interactive) adds vectors mid-project, retires stuck ones, and pivots (retire
several, seed replacements). Vectors live as
...
2. **The rendered PDF** `manuscript/proof.pdf` — committed and rebuilt with
   ...
   Opens inline on GitHub. (Status/chore/findings-only commits don't trigger a
   rebuild.)
3. **The Issues tab** — every subgoal. The ROADMAP uses task-list syntax
   (`- [ ] #N <title>`) so checkboxes auto-tick when linked issues close, for
   an N/M meter at a glance.
...
valuable; the label just lets a reader tell a genuine contribution from a
re-proof. It flows from the gate into the
...
The per-clone git identity and GitHub username are set by **`make init`** — a
one-time substitution run immediately after cloning (see the README's "Setup"
section). After it, none of the placeholders (`GH_USERNAME`, `GIT_USER_NAME`,
`GIT_USER_EMAIL`) should remain in bracketed form anywhere tracked. Verify:
...
and `gh issue` resolve against it, and the wrong identity leaks the wrong email
into commit history:
...
`https://github.com/<GH_USERNAME>/<repo-name>.git` — read it from `git remote -v`.
...
Claude, Anthropic, or this tool in commits or PRs. The
...
lemmas with proofs, optional discussion, optional `Formal verification`
appendix. Scaffolding does **not** appear: attack vectors /
...
citing Lean 4 + Mathlib commit SHA, and update the abstract and appendix.
Full policy with examples in SKILL.md.
...
| (optional) `sage` | `polyrith` backend | `brew install --cask sage` — install on demand if formalist needs sage-backed polynomial search |
